...
for (const input of pins.inputs) add(`IN-${input.gpio}`,'Verified dry contact or isolated conditioned output; actual source TBD',
  ...
  input.gpio==='PB1' ? 'All four conditioned healthy channels combined; any alarm, open wire or power loss must assert fault. Never chain raw ALM outputs into PB1. No board pull-up/RC: interface board adds series R, TVS, 3.3 V pull-up and RC. Qualify before coupled dual-Y motion.'
    : /^PG1[2-5]$/.test(input.gpio) ? 'Optional per-axis wiring for a future firmware candidate; not read by the current firmware; no protective stop credit'
    : input.gpio==='PF3' ? 'Safety-relay contact closed while the relay is energized, open on E-stop/trip/broken wire (not an NC auxiliary). E-stop pressed and wire unplugged both read Pn:E; never invert $14 to suit a wrong contact.'
    : input.gpio==='PB2' ? 'Onboard SW2 (BOOT1) shares this net: guard or remove it. No board pull-up/RC: interface board adds series R, TVS, 3.3 V pull-up and RC. Physical header orientation and truth table.'
    : input.gpio==='PB7' ? 'No board pull-up/RC: interface board adds them. Not fail-safe (open circuit reads untriggered): trigger test before every probing cycle. Physical header orientation and truth table.'
