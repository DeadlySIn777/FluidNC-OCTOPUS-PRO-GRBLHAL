...
Historical root diagnostic scripts and generic wiring/configuration files are retained as source history only; they are not MR1 commissioning commands and may write settings or move hardware. Use the current native application and reviewed procedure. The original interactive wiring-map, vendor PDFs/images, build output and capture journals are excluded.
