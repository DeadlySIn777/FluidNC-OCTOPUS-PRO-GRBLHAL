...
// TMC2209 standard baud rate - 100kbaud as used by working rogerlz fork
#define SWS_BAUDRATE            100000
#define ABORT_TIMEOUT           5           // ms (rogerlz uses 5)
    ...
    if(rx_buf.sampling) {
       ...
       bool inbit = DIGITAL_IN(active_uart->port, active_uart->pin);
        ...
        if(rx_buf.bit_count == -1) {                                // -1 means waiting for START (0)
            ...
            rx_buf.sampling = false;                                // 8: stop bit received, tell foreground
        else {
            rx_byte >>= 1;                                          // shift previous
            if (inbit)
                rx_byte |= 0x80;                                    // OR in new
            if(++rx_buf.bit_count == 8) {                           // Prepare for next bit
...
void tmc_uart_write (trinamic_motor_t driver, TMC_uart_write_datagram_t *dgr)
{
    ...
    // TMC2209 needs time to process write command
    // Without this delay, writes may not take effect
    volatile uint32_t dly = 5000;
    while(--dly);
}
...
// Force UART pins HIGH to ensure TMC2209 enters UART mode on power-up
// TMC2209 samples PDN_UART during power-on: HIGH = UART mode, LOW = standalone mode
// BTT drivers have 20K pulldowns that would otherwise force standalone mode
static void force_uart_pins_high (void)
{
    ...
    // Enable ALL GPIO clocks that might be used for UART pins
    __HAL_RCC_GPIOC_CLK_ENABLE();
    __HAL_RCC_GPIOD_CLK_ENABLE();
    __HAL_RCC_GPIOE_CLK_ENABLE();
    __HAL_RCC_GPIOF_CLK_ENABLE();

#ifdef MOTOR_UARTX_PORT
    GPIO_InitStruct.Pin = (1 << MOTOR_UARTX_PIN);
    HAL_GPIO_WritePin(MOTOR_UARTX_PORT, (1 << MOTOR_UARTX_PIN), GPIO_PIN_SET);
    HAL_GPIO_Init(MOTOR_UARTX_PORT, &GPIO_InitStruct);
#endif
#ifdef MOTOR_UARTY_PORT
    GPIO_InitStruct.Pin = (1 << MOTOR_UARTY_PIN);
    HAL_GPIO_WritePin(MOTOR_UARTY_PORT, (1 << MOTOR_UARTY_PIN), GPIO_PIN_SET);
    HAL_GPIO_Init(MOTOR_UARTY_PORT, &GPIO_InitStruct);
#endif
#ifdef MOTOR_UARTZ_PORT
    GPIO_InitStruct.Pin = (1 << MOTOR_UARTZ_PIN);
    HAL_GPIO_WritePin(MOTOR_UARTZ_PORT, (1 << MOTOR_UARTZ_PIN), GPIO_PIN_SET);
    HAL_GPIO_Init(MOTOR_UARTZ_PORT, &GPIO_InitStruct);
#endif
}
...
// Early board initialization for BTT Octopus Pro (ALL variants)
#if defined(BOARD_BTT_OCTOPUS_PRO) || defined(BOARD_BTT_OCTOPUS_PRO_F429) || defined(BOARD_BTT_OCTOPUS_PRO_SLOTS345)

void board_init (void)
{
    // CRITICAL: Force UART pins HIGH before TMC2209 powers up
    // TMC2209 samples PDN_UART at power-on: HIGH = UART mode, LOW = standalone mode
    // BTT drivers have 20K pulldowns that force standalone mode unless we drive HIGH early
    force_uart_pins_high();
    
    // NOTE: tmc_uart_init() is called separately by driver.c
}
#endif

#endif // TRINAMIC_UART_ENABLE == 2

// Provide board_init for non-UART modes (standalone)
#if !defined(TRINAMIC_UART_ENABLE) || TRINAMIC_UART_ENABLE != 2
#if defined(BOARD_BTT_OCTOPUS_PRO) || defined(BOARD_BTT_OCTOPUS_PRO_F429) || defined(BOARD_BTT_OCTOPUS_PRO_SLOTS345)
void board_init (void)
{
