...
// Initialize RS485 direction pin to receive mode (LOW)
static void rs485DirInit (void)
{
    // Configure the GPIO as output and set LOW (receive mode)
    GPIO_InitTypeDef GPIO_InitStructure = {
        .Mode = GPIO_MODE_OUTPUT_PP,
        .Pull = GPIO_PULLDOWN,  // Pull-down to ensure LOW when idle
        .Speed = GPIO_SPEED_FREQ_HIGH,
        .Pin = (1 << RS485_DIR_PIN)
    };
    HAL_GPIO_Init(RS485_DIR_PORT, &GPIO_InitStructure);
    
    // Set HIGH immediately (receive mode - ACTIVE LOW DE/RE)
    DIGITAL_OUT(RS485_DIR_PORT, RS485_DIR_PIN, 1);
}

static void rs485SetDirection (bool tx)
{
    if(!tx) {
        // When switching to RX, wait for transmit complete flag
        while(!(UART0->SR & USART_SR_TC));
        // Delay for RS485 transceiver turnaround (~100us at 180MHz)
        volatile uint32_t delay = 18000;  // ~100us at 180MHz
        while(delay--);
    }
    DIGITAL_OUT(RS485_DIR_PORT, RS485_DIR_PIN, !tx);  // LOW = transmit, HIGH = receive (INVERTED)
}
