...
class ProbeWizard {
    constructor(options = {}) {
        ...
        this.config = {
            ...
            probeMaxZ: -50,       // Max Z probe distance
            probeMaxXY: 30,       // Max XY probe distance
            ...
            probeTimeout: 60000,  // Max time to wait for probe (ms)
    ...
    async probeZSurface() {
        ...
        await this.grbl.sendAndWait(`G91 G0 Z${this.config.safeZ}`);
    ...
    async executeProbe(axis, distance, feedRate) {
        return new Promise((resolve, reject) => {
            ...
            const timeout = setTimeout(() => {
                ...
                reject(new Error(`Probe timeout after ${this.config.probeTimeout / 1000}s`));
            ...
            // Send probe command
            const cmd = `G91 G38.2 ${axis.toUpperCase()}${distance} F${feedRate}`;
    ...
    cancelProbe() {
        if (this.probing) {
            ...
            this.probing = false;
            this.updateStatus('Probe cancelled', 'warning');
        }
