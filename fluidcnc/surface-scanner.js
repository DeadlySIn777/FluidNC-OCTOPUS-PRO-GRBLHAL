...
class SurfaceScanner {
    ...
    async probePoint(x, y) {
        ...
        // Probe down
        const probeResult = await this.grbl.sendAndWait(
            `G38.2 Z${this.bounds.minZ} F${this.config.probeSpeed}`
        );
        
        // Get probe position from grbl state
        await this.waitForProbeResult();
        const probeZ = this.grbl.probeResult?.z ?? this.grbl.state.mpos.z;
        
        // Retract
        await this.grbl.sendAndWait(
            `G0 Z${probeZ + this.config.probeRetract}`
        );
